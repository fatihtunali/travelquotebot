...
export async function GET(
  ...
  try {
    ...
    const accommodations = await query(
      `SELECT
        id, name, city, category, star_rating,
        base_price_per_night, currency, amenities, description, is_active
      FROM accommodations
    ...
    const accommodation = (accommodations as any[])[0];

    return NextResponse.json({
      ...accommodation,
      amenities: accommodation.amenities ? JSON.parse(accommodation.amenities) : null,
    });
  } catch (error: any) {
...
export async function PUT(
  ...
  try {
    ...
    await query(
      ...
      SET name = ?, city = ?, category = ?, star_rating = ?,
          base_price_per_night = ?, currency = ?, amenities = ?,
...
export async function DELETE(
  ...
  try {
    ...
    await query(
      'DELETE FROM accommodations WHERE id = ? AND operator_id = ?',
      [id, operatorId]
    );
