...
export default function TransportDetailPage() {
  ...
  const handleAddPricing = async () => {
    try {
      ...
      const url = editingPriceId
        ? `/api/pricing/transport/${id}/prices/${editingPriceId}`
        : `/api/pricing/transport/${id}/prices`;
  ...
  if (!transport) {
    return <div className="p-6">Transport not found</div>;
  }
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          {transport.type} • {transport.from_location} → {transport.to_location}
        </p>
        <p className="text-gray-600">
          Base Price: {transport.currency} {transport.base_price}
        </p>
      </div>
        ...
        {showAddPricing && (
          <div className="mb-6 p-4 bg-gray-50 rounded border">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Per Person Rate</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Price Per Vehicle (Optional)</label>
                <input
                  type="number"
                  value={newPricing.price_per_vehicle || ''}
                  onChange={(e) => setNewPricing({ ...newPricing, price_per_vehicle: e.target.value ? parseFloat(e.target.value) : null })}
                  className="w-full px-3 py-2 border rounded"
                  placeholder="Use instead of per-person pricing"
                />
              </div>
              <div>
          ...
          <table className="min-w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-right text-sm font-medium">Per Person</th>
                <th className="px-4 py-2 text-right text-sm font-medium">Per Vehicle</th>
            ...
            <tbody className="divide-y">
              {pricingPeriods.map((pricing) => (
                <tr key={pricing.id}>
                  ...
                  <td className="px-4 py-3 text-right">{pricing.pp_dbl_rate > 0 ? `$${pricing.pp_dbl_rate}` : '-'}</td>
                  <td className="px-4 py-3 text-right">{pricing.price_per_vehicle ? `$${pricing.price_per_vehicle}` : '-'}</td>
